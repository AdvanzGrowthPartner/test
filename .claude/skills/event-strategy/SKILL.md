---
name: event-strategy
description: "Anticipa y previsualiza campañas de eventos ecommerce DTC (CyberDay, CyberMonday, Black Friday, Hot Sale, Navidad, Madre/Padre, 18, liquidaciones y eventos propios). ARRANCA con un intake de preguntas y confirma la fecha del evento por web ANTES de traer data; luego lee por MCP (Shopify + búsquedas + Meta) peaks pasados, tráfico, zonas, recurrencia y catálogo, clasifica productos (ganador/sidekick/fantasma/zombie), fija el lead-time, distingue estacional vs always-on, puntúa el leverage y arma un ARTIFACT visual (Gantt, roles, tendencia, proyección). NO ejecuta: previsualiza para que el consultor apruebe. También arma una LANDER pública del evento con formulario de captura. Usa cuando digan: 'prepara el cyber/black/navidad de [cliente]', 'qué evento se viene', 'nos alcanza el tiempo', 'arma la estrategia del evento', 'proyecta el evento vs el año pasado', 'qué productos/canales muevo', 'arma la lander del evento'. ANTES del evento; EN VIVO → ecomm-cyber-audit. SOLO ecommerce DTC/B2C."
---
